...
class MultiFileWriter:
    ...
    def write(self, b, dir_name=POSTINGS_GCP):
        locs = []
        while len(b) > 0:
            pos = self._f.tell()
    ...
    def read(self, locs, n_bytes):
        ...
        for f_name, offset in locs:
            if f_name not in self._open_files:
                # uncomment for colab
                self._open_files[f_name] = fs.open(f_name, 'rb')
...
TUPLE_SIZE = 6       # im going to pack the doc_id and tf values in this 
                     # many bytes.
...
class InvertedIndex:  
    ...
    def read_index(base_dir, name):
        # uncomment for colab
        with fs.open(Path(base_dir) / f'{name}.pkl', 'rb') as f:
    ...
    @staticmethod
    def write_a_posting_list(b_w_pl, bucket_name, dir_name=POSTINGS_GCP):
        ...
        with closing(MultiFileWriter(".", bucket_id, bucket_name)) as writer:
            for w, pl in list_w_pl: 
                ...
                b = b''.join([(doc_id << 16 | (tf & TF_MASK)).to_bytes(TUPLE_SIZE, 'big')
                              for doc_id, tf in pl])
                # write to file(s)
                locs = writer.write(b, dir_name)
                # save file locations to index
                posting_locs[w].extend(locs)
            writer.upload_to_gcp() 
            InvertedIndex._upload_posting_locs(bucket_id, posting_locs, bucket_name, dir_name)
        return bucket_id

    
    @staticmethod
    def _upload_posting_locs(bucket_id, posting_locs, bucket_name, dir_name=POSTINGS_GCP):
        with open(f"{bucket_id}_posting_locs.pickle", "wb") as f:
            pickle.dump(posting_locs, f)
        client = storage.Client()
        bucket = client.bucket(bucket_name)
        blob_posting_locs = bucket.blob(f"{dir_name}/{bucket_id}_posting_locs.pickle")
